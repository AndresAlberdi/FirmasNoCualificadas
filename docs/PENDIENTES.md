...
| :-- | :---- | :---- |
| T-21 | **La respuesta heredada de `POST /v1/signing-sessions/{id}/confirm` no declara si el sello es cualificado** | Su `TimestampInfo` publica autoridad, serie y hora, y nada más: un integrador que lea esa respuesta no distingue un sello de prueba de uno de fecha cierta sin analizar el token. **La evidencia sí lo declara**: la pista de auditoría registra `tsa_qualified` y el expediente deja de afirmar un prestador cualificado cuando el sello es de prueba — que es lo que se exhibe. No se agregó a la respuesta porque cambia el contrato público de una superficie prevista para retirarse (el contrato v1 se montó en un router aparte justamente para poder quitarla), y el SDK no la consume. Se cierra retirando `/v1/signing-sessions` o, si se decide conservarla, agregando el campo como cambio compatible con su `api/openapi.yaml` regenerado |
