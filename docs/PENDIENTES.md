...
| T-25 | **El script de prueba compensa defectos del bloque visible** | En `main`, el bloque del #22 sale ilegible —la declaración va en una sola línea y pyHanko escala todo el bloque, QR incluido—, imprime «ƒ» en lugar de «…» y no lleva la marca de entorno. Lo corrige el PR #47 (`fix/bloque-constancia-legible`). Cuando se fusione, retirar `FirmanteMarcado` y `CajaExacta` del script y volver a correrlo |
