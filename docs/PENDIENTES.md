...
| :-- | :---- | :---- |
| T-22 | **La respuesta heredada de `POST /v1/signing-sessions/{id}/confirm` no declara si el sello es cualificado** | Su `TimestampInfo` publica autoridad, serie y hora, y nada más: un integrador que lea esa respuesta no distingue un sello de prueba de uno de fecha cierta sin analizar el token. **La evidencia sí lo declara**: la pista de auditoría registra `tsa_qualified` y el expediente deja de afirmar un prestador cualificado cuando el sello es de prueba — que es lo que se exhibe. No se agregó a la respuesta porque cambia el contrato público de una superficie prevista para retirarse (el contrato v1 se montó en un router aparte justamente para poder quitarla), y el SDK no la consume. Se cierra retirando `/v1/signing-sessions` o, si se decide conservarla, agregando el campo como cambio compatible con su `api/openapi.yaml` regenerado |
...
| T-24 | **La hoja de firma existe solo en el script de prueba** | `scripts/firmar-prueba-alianza.py --hoja-de-firma` agrega una página final para las firmas como actualización incremental: la huella del archivo original, el bloque del firmante y un espacio reservado para la firma cualificada posterior. Se usó en la prueba con Alianza (2026-09-11) porque el documento no tenía lugar libre para el bloque. Para entrar al motor necesita ADR: cambia **qué documento se cierra y se hashea** (regla #3) —el bloque declara la huella del original más la hoja, no la del archivo que envió el tenant— y toca el contrato v1. Es una decisión de producto, no una obligación legal |
| T-26 | **Falta el resultado de la prueba con el firmador de Alianza** | El PDF de prueba se entregó el 2026-09-11 con la firma no cualificada de un cliente ficticio. Localmente quedó comprobado —con pyHanko y con `pdfsig`— que una segunda firma incremental no invalida la primera. Falta saber si **su** firmador acepta un PDF con una firma previa emitida por una CA no confiable (si la rechaza, se le entrega el certificado de la CA de desarrollo), dónde estampa la suya y si agrega sello de tiempo, que en la reunión del 03-sep quedó sin confirmar. El resultado se registra acá |
| T-21 | **Datos del tenant con caracteres que la fuente del bloque no codifica** | El bloque usa Courier estándar, que el visor lee en WinAnsi, y pyHanko escribe el texto en PDFDocEncoding: coinciden en ASCII y en las letras acentuadas, y difieren en signos tipográficos —«…» sale «ƒ»; el apóstrofo tipográfico de un apellido, las rayas y las comillas curvas también se desplazan—, y lo que no entra en PDFDocEncoding sale en UTF-16, ilegible. Los rótulos del perfil y la huella ya son ASCII, y `test_constancia_visible.py::TestLaFuenteCodificaLoQueSeImprime` lo verifica; **los datos del firmante, no**: el motor imprime lo que recibe. Hay dos salidas y ninguna es de maquetación: normalizar a ASCII —cambia el nombre que el bloque declara— o incrustar una fuente con cobertura Unicode. Se decide con T-19, que es cuando esos datos empiezan a llegar desde la transacción |
| T-16 | **Falta verificar un despliegue real en la cuenta de un cliente** | Resuelto lo documental y lo de nombres: el contrato de despliegue está en `docs/DESPLIEGUE-DEL-CLIENTE.md`, el prefijo de recursos es la variable `resource_prefix` (por defecto `fenc`, que ya no etiqueta al cliente como prestador) y la configuración deriva la cuenta con `aws_caller_identity`. Lo que **no** está comprobado es que un `apply` completo funcione en una cuenta ajena: nunca se aplicó contra ninguna. Hasta hacerlo, «es desplegable por el cliente» es una lectura del código, no un hecho |
| T-17 | **Alojamiento gestionado como servicio limitado, sin construir todavía** | Recomendado y aceptado por dirección como opción comercial futura. **Reabre la obligación de registro del art. 15**: si FNC aloja la plataforma y crea las firmas a cambio de una remuneración, presta un servicio de confianza aunque el cliente firme lo suyo. Exige su propio ADR, la comunicación al MIC con `FOR-ICPP-02` y la conformidad con el `DOC-ICPP-20` — es decir, revive B-02, B-03 y los pendientes P-*. No se empieza antes del dictamen de L-05 |
| T-18 | **Elegir la autoridad de sellado de tiempo del entorno de desarrollo** | Existen TSA públicas gratuitas conformes a RFC 3161 (FreeTSA, DigiCert, Sectigo) y es posible operar una propia sobre KMS con `sigstore/timestamp-authority`. **Ninguna sirve para fecha cierta en Paraguay**: el art. 63.1.b exige un sello de un *prestador cualificado*, y ninguna de ellas lo es acá. Sirven para desarrollo y pruebas, donde el acta ya declara `timestamp.qualified: false` |
...
| T-14 | **El visor forense sigue con datos sintéticos, y no puede dejar de estarlo todavía** | La verificación pública **ya consume la API real**: `GET /v1/verify/{code}` y `/.well-known/fnc-keys.json` no exigen credenciales, así que el panel trae la constancia, verifica su acta con WebCrypto y contrasta ambas. El expediente forense es otra cosa: la API B2B se autentica con **HMAC sobre un secreto compartido por inquilino**, y ese secreto no puede vivir en un navegador — ponerlo ahí lo entregaría a cualquiera que abra las herramientas de desarrollo, junto con la capacidad de firmar peticiones en nombre del inquilino. Depende de T-15 |
| T-15 | **El panel no tiene sesión propia** | `esquema-dashboard-b2b-pscnc.md` §5.2 prevé Cognito o el proveedor federado del cliente con MFA obligatorio, y no existe: no hay endpoints de sesión del panel ni control de acceso por rol. Es el requisito que bloquea T-14, y no es trabajo de frontend |
